...
public class DiveDetails {
    ...
    private String diveConditionsNA;
    private String diveActivitiesNA;

    //New constructor without array values


    public DiveDetails(int duration, int maxDepth, int avgDepth, int temp, int visibility, int pressureStart, int pressureEnd, int airType, String diveConditionsNA, String diveActivitiesNA) {
        ...
        this.diveConditionsNA = diveConditionsNA;
        this.diveActivitiesNA = diveActivitiesNA;
    }
    ...
    DiveDetails(int duration, int maxDepth, int avgDepth, int temp, int visibility, int pressureStart,
                int pressureEnd, String[] diveConditions, String[] diveActivities, boolean isNaui) {
        ...
        setPressureGroup(isNaui);
    }

    //Constructor for diving on Nitrox to capture air type percentage
    public DiveDetails(int duration, int maxDepth, int avgDepth, int temp, int visibility, int pressureStart,
                       int pressureEnd, int airType, String[] diveConditions, String[] diveActivities, boolean isNaui) {
        setDuration(duration);
        setMaxDepth(maxDepth);
        setAvgDepth(avgDepth);
        setTemp(temp);
    ...
    private void setPressureGroup(boolean isNaui) {
        if(isNaui)
            this.pressureGroup = new NauiDiveTable(NauiDiveTable.calcEAD(airType, maxDepth), duration, 0).getPG();
        else
            this.pressureGroup = new PadiTables(PadiTables.calcEAD(airType, maxDepth), duration).getPG();
    }
    ...
    public String getDiveConditionsNA() {
        return diveConditionsNA;
    }

    public void setDiveConditionsNA(String diveConditionsNA) {
        this.diveConditionsNA = diveConditionsNA;
    }

    public String getDiveActivitiesNA() {
        return diveActivitiesNA;
    }

    public void setDiveActivitiesNA(String diveActivitiesNA) {
        this.diveActivitiesNA = diveActivitiesNA;
    }
